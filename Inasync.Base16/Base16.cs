...
namespace Inasync {
    ...
    public static class Base16 {
        ...
        public static int GetEncodedLength(int bytesLength) => bytesLength * 2;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int GetDecodedLength(int charsLength) => charsLength / 2;
        ...
        public static string Encode(ReadOnlySpan<byte> bytes, bool toUpper = false) {
            ...
            var charsLength = GetEncodedLength(bytes.Length);
            char[]? charArray = null;
            try {
                Span<char> chars = charsLength <= _stackallocThreshold ? stackalloc char[charsLength] : (charArray = ArrayPool<char>.Shared.Rent(charsLength));
        ...
        public static bool TryEncode(ReadOnlySpan<byte> bytes, Span<char> chars, out int charsWritten, bool toUpper = false) {
            ...
            if (chars.Length < GetEncodedLength(bytes.Length)) {
                charsWritten = 0;
                return false;
            }
        ...
        public static bool TryDecode(string? input, out byte[]? result) {
            ...
            var bytesLength = GetDecodedLength(input.Length);
            var byteArray = new byte[bytesLength];
            if (!TryDecode(input.AsSpan(), byteArray, out var bytesWritten)) {
                result = null;
                return false;
            }
        ...
        public static bool TryDecode(ReadOnlySpan<char> chars, Span<byte> bytes, out int bytesWritten) {
            ...
            if (bytes.Length < GetDecodedLength(chars.Length)) { return false; }
